...
function getGroups {
  ...
  logThis "Executing command:  [curl -X 'GET' -o $accountDir/response.json \"${CONF_aria_operations_url}/api/v2/usergroup\" -H 'accept: application/json' -H \"Authorization: Bearer  ${api_token}]\"" "DEBUG"
  curl -X 'GET' -o $accountDir/response.json \
    "${CONF_aria_operations_url}/api/v2/usergroup/${groupID}" \
    -H 'accept: application/json' \
    -H "Authorization: Bearer  ${api_token}" && logThis "Successfully retrieved group list." "INFO" || logThis "Could not retrieve group list." "CRITICAL"
